...
- `src/app/features/<feature>/` — one folder per business feature: `pages/` (one folder per page, named `<verb>-<entity>-page`), `services/` (the feature's logic, wrapping the generated client and owning `rxResource` state — pages never import `core/api` directly), `forms/` (models + validation schemas), `<feature>-routes.ts`.
