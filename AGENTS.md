...
- `src/app/features/<feature>/` — one folder per business feature: `pages/` (one folder per page, named `<entity>-<action>-page`), `services/` (one service per feature, a thin data-access layer wrapping the generated client — one method per backend call, no business logic — and owning `rxResource` state; pages never import `core/api` directly), `forms/` (models + validation schemas), `<feature>-routes.ts`.
...
- File names separate words with dashes, never dots: `logger-service.ts`, `users-routes.ts` and `app-config.ts`, not `logger.service.ts`, `users.routes.ts` or `app.config.ts` (this departs from what `ng new` generates). Services end in `-service`; components are named after their role instead (`home-page.ts`, `not-found-page.ts`). `core/api/` is the exception: it is generated by Orval and keeps its own naming.
