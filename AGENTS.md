...
- `src/app/features/<feature>/` — one folder per business feature: `pages/` (one folder per page, named `<entity>-<action>-page`), `services/` (one service per page, named after it, wrapping the generated client and owning `rxResource` state — pages never import `core/api` directly), `forms/` (models + validation schemas), `<feature>-routes.ts`.
