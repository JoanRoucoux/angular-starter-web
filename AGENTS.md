...
- File names separate words with dashes, never dots: `users-routes.ts` and `app-config.ts`, not `users.routes.ts` or `app.config.ts` (this departs from what `ng new` generates). No Angular type suffix either (`logger.ts`, `home-page.ts`, not `logger.service.ts`). `core/api/` is the exception: it is generated by Orval and keeps its own naming.
